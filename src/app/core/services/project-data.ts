...
/** The list is configurable (Budget Settings); these four are the starting values. */
...
export const CURRENCIES = ['OMR', 'USD'] as const;
export type Currency = (typeof CURRENCIES)[number];
/** Fixed demo rate; OMR is pegged to the dollar. */
...
export interface ProjectRequest {
  ...
  from?: string;
  to?: string;
  /** Total estimated project cost, in `currency`. */
  budget: number;
  currency: Currency;
  ...
  resourceType?: 'New' | 'Existing';
  resourceSource?: 'Internal' | 'Outsourced';
  comments?: string;
  attachments: string[];
  requestedBy: string;
...
export const fx = (p: Pick<ProjectRequest, 'currency'>) => (p.currency === 'USD' ? USD_TO_OMR : 1);
...
function seed(id: string, name: string, projectStatus: ProjectStatus, scope: string, budget: number, reason: string, status: SubmissionStatus, priority: Priority, extra: Partial<ProjectRequest> = {}, note = ''): ProjectRequest {
  ...
  const history: ProjectEvent[] = [ev(-40, TL, 'Line Manager', 'Created', 'Project added.')];
  if (status !== 'Draft') history.push(ev(-35, TL, 'Line Manager', 'Submitted', 'Sent for a decision.'));
  ...
  return {
    id, financialYear: CURRENT_FY, currency: 'OMR', department: 'Customer Care Unit', name, projectStatus, scope, budget, reason, priority, projectManager: PM, lineManager: TL, headCount: 0, attachments: [], requestedBy: TL, status,
