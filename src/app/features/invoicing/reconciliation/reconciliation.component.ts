                  ...
                  <td colspan="6" class="!border-t-0 !bg-surface-subtle !whitespace-normal !pt-0">
                    @if (l.source === 'wfo') {
                      ...
                      <p class="text-[11px] text-ink-400 mt-2">
                        @switch (l.component) {
                          @case ('performance') { Each eligible agent's fixed monthly performance amount (eligible: score above {{ store.payrollRules().omaniMinScore }}% Omani, {{ store.payrollRules().nonOmaniMinScore }}% non-Omani) — amounts and thresholds are set in <a class="text-brand-600 font-medium" routerLink="/admin/performance-settings">Performance Settings</a>. }
                          @case ('overtime') { Overtime hours &times; basic &divide; {{ store.payrollRules().overtimeDays }} days &divide; {{ store.payrollRules().overtimeHoursPerDay }} hours &times; {{ store.payrollRules().overtimePremium }}, per agent — set in <a class="text-brand-600 font-medium" routerLink="/admin/overtime-settings">Overtime Settings</a>. }
