...
export class ContractTrendsComponent {
  ...
  charts = computed<ChartDef[]>(() => {
    ...
    return [
      ...
      { title: 'PO Value and Contract Amount by Contract', subtitle: 'OMR — PO value always equals the contract amount', type: 'bar' as const, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' as const, labels: { boxWidth: 10, font: { size: 11 } } } }, scales: { y: { beginAtZero: true } } }, data: { labels: cs.map((c) => c.reference.slice(-8)), datasets: [{ label: 'Contract amount', data: cs.map((c) => c.amount), backgroundColor: C.brand }, { label: 'PO value', data: cs.map((c) => this.ops.purchaseOrdersOf(c).reduce((s, p) => s + (p.amount ?? 0), 0)), backgroundColor: C.orange }] } },
